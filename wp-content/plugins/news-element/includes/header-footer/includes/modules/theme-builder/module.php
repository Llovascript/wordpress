...
class Module extends \Elementor\Core\Base\Module {
	...
	public function localize_settings( $settings ) {
		$settings = array_replace_recursive( $settings, [
			'i18n' => [
				'publish_settings' => __( 'Publish Settings', 'news-element' ),
				'conditions' => __( 'Conditions', 'news-element' ),
				'display_conditions' => __( 'Display Conditions', 'news-element' ),
				'choose' => __( 'Choose', 'news-element' ),
				'add_condition' => __( 'Add Condition', 'news-element' ),
				'conditions_title' => __( 'Where Do You Want to Display Your %s?', 'news-element' ),
				'conditions_description' => __( 'Set the conditions that determine where your %s is used throughout your site.', 'news-element' ) . '<br>' . __( 'For example, choose \'Entire Site\' to display the template across your site.', 'news-element' ),
				'conditions_publish_screen_description' => __( 'Apply current template to these pages.', 'news-element' ),
				'save_and_close' => __( 'Save & Close', 'news-element' ),
				'open_site_editor' => __( 'Open Site Editor', 'news-element' ),
				'view_live_site' => __( 'View Live Site', 'news-element' ),
			],
            'urls' => [
	...
	public function print_post_type_field() {
		...
		<div id="elementor-new-template__form__post-type__wrapper" class="elementor-form-field">
			<label for="elementor-new-template__form__post-type" class="elementor-form-field__label">
				<?php echo __( 'Select Post Type', 'news-element' ); ?>
			</label>
			<div class="elementor-form-field__select__wrapper">
				<select id="elementor-new-template__form__post-type" class="elementor-form-field__select" name="<?php echo Single::REMOTE_CATEGORY_META_KEY; ?>">
					...
					// 404.
					echo sprintf( '<option value="%1$s">%2$s</option>', 'not_found404', __( '404 Page', 'news-element' ) );
	...
	public function add_finder_items( array $categories ) {
		$categories['create']['items']['theme-template'] = [
			'title' => __( 'Add New Theme Template', 'news-element' ),
			'icon' => 'plus-circle-o',
	...
	public function admin_menu() {
		add_submenu_page( 'edit.php?post_type=elementor_library', '', __( 'Theme Builder', 'news-element' ), 'publish_posts', $this->get_admin_templates_url( true ) );
	}
	...
	public function print_new_theme_builder_promotion( $views ) {
		...
		if ( self::ADMIN_LIBRARY_TAB_GROUP === $current_tab_group ) {
			...
			$admin_notices->print_admin_notice( [
				'title' => __( 'Meet the New Theme Builder: More Intuitive and Visual Than Ever', 'news-element' ),
				'description' => __( 'With the new Theme Builder you can visually manage every part of your site intuitively, making the task of designing a complete website that much easier', 'news-element' ),
				'button' => [
					'text' => __( 'Try it Now', 'news-element' ),
